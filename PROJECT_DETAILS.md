...
- `images`  
  Architecture diagram used in root `README.md`.
...
- Hardcoded service URLs are present in frontend/backend routes (e.g., `54.193.65.42`, `3.15.28.161`), so deployment/config centralization is limited.
...
- `OpenAI-service` and `transcription_service` both include dependencies that are broader than their current used code paths.
...
- Some route/proxy leftovers in frontend (`src/services/api.js`) do not match active production URLs.
...
- Error paths in async consumers mostly log and continue; no dead-letter queue flow documented in code.
...
## 11. Temporal Integration (New)

Temporal is now introduced as orchestration for the main upload-to-story-generation path without removing your existing S3/Lambda/SQS pipeline.

### New component

- `temporal_service`
    - API (`src/index.js`): starts and inspects workflows.
    - Worker (`src/worker.js`): executes workflows and activities.
    - Workflow (`src/workflows.js`): waits for stories to be generated for a given user/project.
    - Activity (`src/activities.js`): calls `OpenAI-service /api/getActiveStories`.

### How it connects

1. `backend_service` upload route now calls:
    - `POST {TEMPORAL_SERVICE_BASE_URL}/workflows/audio-processing/start`
2. Temporal workflow `audioProcessingWorkflow` starts per upload.
3. Workflow polls active stories through activity.
4. Workflow completes when `active_stories > 0`, or times out after the configured checks.

### Current behavior

- Existing async infrastructure still does the heavy processing:
    - S3 event -> Lambda -> transcription queue -> transcription service -> story queue -> OpenAI story generation.
- Temporal adds durable state tracking/orchestration around completion detection.

### New env variable required in `backend_service`

- `TEMPORAL_SERVICE_BASE_URL`  
